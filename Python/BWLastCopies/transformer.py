...
class CreateNotification:
    ...
    def create_notification(self, message: list, iterate: bool = False) -> str:
        ...
        if iterate:
            notilist = self.create_notification_iterated(notilist)
        # print(notilist)
        notification = ''.join(notilist)
        # print(notification)
        return notification
    
    def create_notification_iterated(self,notification:list[dict])->list[dict]:
        """
        create a notification with iterated issues, libraries and counts. This cleans up duplicate entries. 
        
        
        
        Args:
            - notification (list[dict]): The list of the results of the query, which is a list of dictionaries.
        
        Returns:
            - clean_list (list[dict]): the clean list of dictionaries in a list
        """
        notification_iterated=[]
        def compare_issues(current_issue,next_issue):
            if current_issue == next_issue:
                return True
            else:
                return False
        def remove_duplicates(libraries):
            for lib in libraries:
                if lib in libraries:
                    libraries.remove(lib)
            return libraries
        for index,data in enumerate(notification):
            new_notification={}
            if not index+1 == len(notification):
                current_issue=notification[index]['issue']
                next_issue=notification[index+1]['issue']
                print(f'current_issue:{current_issue}, next_issue:{next_issue}')
                if compare_issues(current_issue,next_issue):
                    print(f'{current_issue} == {next_issue}')
                    # new_notification['issue']=current_issue
                    # new_notification['count']=notification[index]['count']+notification[index+1]['count']
                    # new_notification['libraries']=notification[index]['libraries']+notification[index+1]['libraries']
                    notification[index]['libraries'].extend(notification[index+1]['libraries'])
                    notification[index]['libraries']=remove_duplicates(notification[index]['libraries'])
                    notification[index]['count']+=notification[index+1]['count']
                    notification[index]['change']=True
                    notification_iterated.append(notification[index])
                    
            elif index+1 > len(notification):
                current_issue=notification[index]['issue']
                next_issue=notification[index]['issue']
                print(f'current_issue:{current_issue},next_issue:{next_issue}')
        print(f'notification_iterated:{notification_iterated}')
...
if __name__ == '__main__':
    ...
    print(transf.create_notification_iterated(currlist))