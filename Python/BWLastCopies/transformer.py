...
class CreateNotification:
    ...
    def create_notification_iterated(self,notification:list[dict])->list[dict]:
        """
        Takes a list of dictionaries and returns a list of dictionaries with the same issue combined into one dictionary.      

        Args:
            - notification (list[dict]): The list of dictionaries.
        
        Returns:
            - list[dict]: cleaned up list of dictionaries.
        """
        message = self.create_clean_list(notification)
        notification_length = len(message)
        issues = []
        counts = []
        libraries = []
        ...
        notification = ''.join(notilist)
        return notification
    def create_clean_list(self,list_to_clean):
        new_list=[]
        for i in list_to_clean:
            for j in new_list:
                if i["issue"]==j["issue"]:
                    j["count"]+=i["count"]
                    j["libraries"].extend(i["libraries"])
                    break
            else:
                new_list.append(i)
        for i in new_list:
            ...
            i.update({"libraries":library_list})
    ...
    def test_iterate(self, list_to_iterate:list)->list[dict]:
        result_list=[]
        try:
            for id,entry in enumerate(list_to_iterate):
                if list_to_iterate[id]['issue'] == list_to_iterate[id+1]['issue']:
                    result_list.append(list_to_iterate[id])
        except IndexError:
            pass
        for id in list_to_iterate:
            for entry in result_list:
                if entry['issue'] == id['issue']:
                    entry['count']+=id['count']
                    entry['libraries'].extend(id['libraries'])
                    #list_to_iterate.remove(id)
        print(result_list)
if __name__ == '__main__':
    ...
    notification=transf.create_notification_iterated(currlist)
